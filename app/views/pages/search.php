...
<body>
      ...
      <div class="navbar-nav">
        <div class='menu-compta-articles'>
          <div class='compta-articles'>
            ...
            <?php if (($_SESSION['data_user']->{'role'} != 'ROLE_USER') && ($_SESSION['data_user']->{'role'} != 'ROLE_ADMIN')) {
        ...
        <li>
          <a>
            <div id='user__toggle' class="user">
              <p class="user__user"><?php if (($_SESSION['data_user']->{'ecn_year'} == NULL)) : ?><span class="notif"></span><?php endif; ?>Bonjour
              ...
              <div class="user__submenuProfile">
                <img class="profile_img" src="<?= '/img/' . $_SESSION['data_user']->{'avatar'} ?>" height="50px" width="50px" style="border-radius:50%" />
                <p class="user__name position-relative">
                  <?php echo $_SESSION['data_user']->{'firstname'}; ?>
                </p>
              </div>
              <?php if (($_SESSION['data_user']->{'ecn_year'} == NULL)) : ?> <a class="user__link" style="color: crimson" href="/pages/profil"><img src='/assets/icon_proto/alert.svg' height='30px' />Compléter votre profil</a><?php endif; ?>
      ...
      <div class="container-fluid container-search first">
        <div class="row category__row">
          <div class="col-xs-12 col-sm-6 col-lg-4 offset-sm-0 category_col">
            <div class="category">
              <a class="category__link category__link--active" href="/pages/fiche"><?php if ($data['notifFiche']) : ?><span class='notif'></span><?php endif; ?>Fiches</a>
              <a class="category__link" href="/pages/exam"><?php if ($data['notifBilan']) : ?><span class='notif'></span><?php endif; ?>Bilans</a>
              <a class="category__link" href="/pages/annuaire"><?php if ($data['notifAnnuaire']) : ?><span class='notif'></span><?php endif; ?>Annuaire</a>
          ...
          <div class="col-sm-12 col-md-12 col-lg-4  text-center mt-2 mx-auto">
            ...
            <div id="editAndAddTemplate">
              <nav class="navbar bg-white">
                <input type="search" class="form-control mr-sm-2 flex-grow-1" name="medicalFolder" id="searchMedicalFolder" placeholder="Recherche rapide" autocomplete="off" autofocus>
                <div class="clearable">
                ...
                <div class="list-group" id="folders">
                  <?php foreach ($data['medicalFolders'] as $folder) : ?>
                    <a id="folder-<?php echo $folder->{'id'}; ?>" class="folder list-group-item d-flex justify-content-start align-items-center btn btn-light"><img src="/assets/icon_proto/Icon - File-Folder.svg" height="20px" />
                      <?php echo $folder->{'title'}; ?><?php if ($folder->{'notif'} == 1) : ?><span class="notif"></span><?php endif; ?><span class="badge <?= ($folder->{'admin_medicalFolder_id'} === null) ? 'badge-danger' : 'badge-primary'; ?> badge-pill">
  ...
  <!-- modal move file -->
  <!-- Modal -->
  <div class="modal fade" id="moveFileModal" tabindex="-1" role="dialog" aria-labelledby="moveFileModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Déplacer la fiche</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form method="POST" action="/single/moveFileToFolderById" autocomplete="off">
            <input style="display: none" name="currentFile" id="currentFileForm" />
            <select id="select_folder" class="form-control" name="folderId">
            </select>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
          <button type="submit" class="btn btn-primary">Déplacer la fiche</button>
        </div>
        </form>
      </div>
    </div>
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="green" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-save"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg>
        ...
        <div id="collapse0" class="notes-input collapse show" aria-labelledby="headingOne" data-parent="#accordion" >
          <div class="card-body">
            <textarea name="text"  oninput="this.style.height = ''; this.style.height = this.scrollHeight +'px'" id="note__0"></textarea>
          </div>
        </div>
      </div>
    `
